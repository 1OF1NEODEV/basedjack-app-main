...
import { usePayContext } from './PayProvider.js';
...
function PayButton({
  ...
  const _usePayContext = usePayContext(),
    ...
    onSubmit = _usePayContext.onSubmit;
  const iconSvg = useIcon({
    icon
  });
  const isLoading = lifecycleStatus?.statusName === PAY_LIFECYCLESTATUS.PENDING;
  const isFetchingData = lifecycleStatus?.statusName === PAY_LIFECYCLESTATUS.FETCHING_DATA;
  const isDisabled = disabled || isLoading || isFetchingData;
  const buttonText = useMemo(() => {
    if (lifecycleStatus?.statusName === PAY_LIFECYCLESTATUS.SUCCESS) {
      return 'View payment details';
    }
    if (lifecycleStatus?.statusName === PAY_LIFECYCLESTATUS.ERROR && lifecycleStatus?.statusData.error === 'User has insufficient balance') {
      return 'Get USDC';
    }
    return text$1;
