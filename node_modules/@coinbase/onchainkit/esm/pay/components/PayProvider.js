...
import { PAY_LIFECYCLESTATUS, GENERIC_ERROR_MESSAGE, PayErrorCode, NO_CONNECTED_ADDRESS_ERROR, PAY_INSUFFICIENT_BALANCE_ERROR_MESSAGE, PAY_INSUFFICIENT_BALANCE_ERROR, NO_CONTRACTS_ERROR, USER_REJECTED_ERROR } from '../constants.js';
...
const emptyContext = {};
const PayContext = /*#__PURE__*/createContext(emptyContext);
function usePayContext() {
...
function PayProvider({
  ...
  useEffect(() => {
    if (lifecycleStatus.statusName === PAY_LIFECYCLESTATUS.INIT && address && !fetchedDataHandleSubmit.current) {
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      if (lifecycleStatus.statusName === PAY_LIFECYCLESTATUS.ERROR && lifecycleStatus.statusData?.code === PayErrorCode.INSUFFICIENT_BALANCE) {
        window.open('https://keys.coinbase.com/fund?asset=USDC&chainId=8453', '_blank', 'noopener,noreferrer');
        // Reset status
        setErrorMessage('');
        updateLifecycleStatus({
          statusName: PAY_LIFECYCLESTATUS.INIT,
      ...
      if (insufficientBalanceRef.current && priceInUSDCRef.current) {
        setErrorMessage(PAY_INSUFFICIENT_BALANCE_ERROR_MESSAGE(priceInUSDCRef.current));
        updateLifecycleStatus({
          statusName: PAY_LIFECYCLESTATUS.ERROR,
          statusData: {
            code: PayErrorCode.INSUFFICIENT_BALANCE,
            error: PAY_INSUFFICIENT_BALANCE_ERROR,
            message: PAY_INSUFFICIENT_BALANCE_ERROR_MESSAGE(priceInUSDCRef.current)
          }
        });
        return;
      }
      ...
      const errorCode = isUserRejectedError ? PayErrorCode.USER_REJECTED_ERROR : PayErrorCode.UNEXPECTED_ERROR;
  ...
  }, [address, chainId, chargeId, connectAsync, fetchData, isConnected, isSmartWallet, isSponsored, lifecycleStatus.statusData, lifecycleStatus.statusName, paymaster, switchChainAsync, updateLifecycleStatus, writeContractsAsync]);
  ...
  return /*#__PURE__*/jsx(PayContext.Provider, {
    value: value,
    children: children
  });
}
export { PayContext, PayProvider, usePayContext };
//# sourceMappingURL=PayProvider.js.map
